...
namespace MAP_Web.Services
{
    public class ApproverDashboardService : IApproverDashboardService
    {
        ...
        public ApproverDashboardService(IUnitOfWork unitOfWork, IStatusService statusService)
        {
        ...
        public async Task<List<DashboardViewModel>> FindAsync()
        {
            ...
            var requests = await this.requestRepo.GetPagedListAsync(include: r => r.Include(rr => rr.NewAffiliation).ThenInclude(n => n.CustomerProfile).Include(m => m.MAEF), orderBy: x => x.OrderByDescending(y => y.Id), predicate: r => r.Status == 8 || r.Status == 30 || r.Status == 31);
            ...
            foreach (var item in requests.Items)
            {
                dashboardContainer.Add(new DashboardViewModel
                {
                    ...
                    processedBy=item.MAEF.processedBy
                });
            }
        ...
        public async Task<List<DashboardViewModel>> GetListAsync(string field, string sortDirection, int pageIndex, int pageSize, string filter)
        {
            ...
            var requests = await this.requestRepo.GetPagedListAsync(
                            include: r => r.Include(rr => rr.NewAffiliation)
                                .ThenInclude(n => n.CustomerProfile).Include(m => m.MAEF),
